...
#[tauri::command]
pub async fn llm_local_model_info(_db: State<'_, AppDbState>) -> Result<LocalModelInfo, String> {
    Ok(LocalModelInfo::lite())
}

/// Preserve the source command contract; Lite does not require a model directory.
#[tauri::command]
pub async fn llm_open_models_dir(_db: State<'_, AppDbState>) -> Result<(), String> {
    Err("No requerido en EntropIA Lite. Configurá OpenRouter en Configuración para usar LLM remoto.".to_string())
}

/// Preserve the source command contract; Lite does not require model downloads.
#[tauri::command]
