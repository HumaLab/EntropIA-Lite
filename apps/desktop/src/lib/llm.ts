 ...
 * Communicates with the Rust LLM backend through OpenRouter.
...
/** Check if OpenRouter is configured and ready to accept LLM jobs. */
export function llmIsAvailable(): Promise<boolean> {
  return invoke<boolean>('llm_is_available')
}

/** Legacy status contract; Lite reports OpenRouter readiness instead of model files. */
...
/** Query the legacy model status contract for OpenRouter-only Lite. */
export function llmLocalModelInfo(): Promise<LocalModelInfo> {
  return invoke<LocalModelInfo>('llm_local_model_info')
}

/** Legacy no-op in Lite; model directories are not required. */
...
/** Legacy no-op in Lite; OpenRouter does not require model downloads. */
