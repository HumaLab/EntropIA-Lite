//! Shared SQL helpers used by the DB browser commands and the sync module.
//!
//! These were promoted verbatim from `db/commands.rs` private scope so the sync
//! apply path (PROTOCOL §"Semántica de apply") can reuse the exact same
//! identifier validation/quoting and JSON→SQL parameter binding without
...
/// underscores. Mirrors the `^[A-Za-z_][A-Za-z0-9_]*$` contract used by the
/// apply path before any identifier is interpolated into SQL.
