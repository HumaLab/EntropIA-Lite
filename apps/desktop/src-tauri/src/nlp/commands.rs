...
pub async fn embedding_open_models_dir(db: State<'_, AppDbState>) -> Result<(), String> {
    let _ = db;
    Err("No requerido en EntropIA Lite. Configurá OpenRouter en Configuración para usar embeddings remotos.".to_string())
}
...
pub async fn embedding_download_model(
    ...
    crate::app_logs::info(
        &app_handle,
        "embedding/download",
        "Descarga de embeddings no requerida en EntropIA Lite; usá OpenRouter remoto.",
    );

    let error = "No requerido en EntropIA Lite. Configurá OpenRouter en Configuración para usar embeddings remotos.".to_string();
    crate::app_logs::error(&app_handle, "embedding/download", error.clone());
...
#[allow(dead_code)]
async fn legacy_embedding_download_task(model_dir: std::path::PathBuf, app_handle: AppHandle) {
        ...
        match result {
            Ok(Ok(())) => {
                crate::app_logs::info(
                    &app_handle,
                    "embedding/download",
                    "Preparación de embeddings completada",
                );
            }
            Ok(Err(error)) => {
                crate::app_logs::error(
                    &app_handle,
                    "embedding/download",
                    format!("Preparación de embeddings falló: {error}"),
                );
                let _ = app_handle.emit(
                    "embedding:download_error",
                    super::embeddings::EmbeddingDownloadErrorPayload { error },
            ...
            Err(error) => {
                let message = format!("Embedding preparation task panicked: {error}");
...
mod tests {
    ...
    fn similar_assets_queries_across_collections_and_excludes_self() {
        ...
        conn.execute(
            "INSERT INTO extractions(asset_id, text_content, created_at) VALUES (?1, ?2, ?3)",
            rusqlite::params![
                "asset-other",
                "Primeras líneas OCR de la página externa",
                4_i64
            ],
        )
        .expect("other asset extraction should insert");
        ...
        .prepare(
            "SELECT v.asset_id, v.item_id, i.title, i.collection_id, a.path, a.type,
                    COALESCE((
                        SELECT text_content
                        FROM (
                            SELECT e.text_content, 0 AS source_order, e.created_at
                            FROM extractions e
                            WHERE e.asset_id = v.asset_id AND COALESCE(e.text_content, '') != ''
                            UNION ALL
                            SELECT t.text_content, 1 AS source_order, t.created_at
                            FROM transcriptions t
                            WHERE t.asset_id = v.asset_id AND COALESCE(t.text_content, '') != ''
                        ) text_sources
                        ORDER BY source_order ASC, created_at DESC
                        LIMIT 1
                    ), '') AS text_preview,
                    v.embedding
             FROM vec_assets v
...
// `decode_embedding_blob` y `cosine_distance` viven en `nlp::vector` para que
// el pipeline RAG pueda reutilizarlos sin duplicar lógica.
