...
migrate_legacy_asset_paths(&db_path, &app_dir)
            ...
            // OCR worker preserves source queue/UI contracts; Lite uses remote GLM-OCR for OCRH.
            OcrQueue::start_worker(db_path.clone(), ocr_receiver, app.handle().clone());

            // LLM queue: OpenRouter-only in Lite.
            ...
            // NLP queue: FTS locally plus remote OpenRouter embeddings/NER.
            ...
            // Transcription queue: AssemblyAI-only in Lite.
...
/// Legacy OCR records use methods like 'paddle', 'paddle_vl', 'pdf_paddle', and 'pdf_paddle_vl'.
